...
Assertions and checkpoints enhance test validation by ensuring the application behaves as expected. Assertions (e.g., assertText, assertTitle) are used to strictly verify critical elements, such as confirming that a success message appears after form submission; if an assertion fails, the test stops immediately. Checkpoints (e.g., verifyText, verifyElementPresent) serve as softer validations, allowing the test to continue even if a condition isn’t met, which is useful for logging and debugging. Adding these ensures more reliable and meaningful test results.

# how did you test each functionaity with different test data
To test the website's features, we used a variety of inputs and interactions to make sure everything was working the way it should. For example, we tested the "Add to Cart" functionality by selecting different products from different categories and changing the quantity to see if the cart updated correctly. We also tried out the product selection options like size and color to check if the right product variation was shown each time. In addition to that, we tested how the site responded when hovering over product images, just to make sure there weren’t any visual glitches or unexpected behavior. We also looked at how the layout adapted by resizing the browser window to simulate both mobile and desktop views. This helped confirm that the site was still usable and looked clean across different screen sizes. By trying out these different combinations, we made sure that the features we tested worked reliably and could handle common user actions without any issues.
